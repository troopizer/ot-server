...
if(msgcontains(msg, 'travel')) then
	selfSay('We provide the travel service, but you must have traveled by your own to the desire location, in order to travel there by our service.', cid)
 	...
 	selfSay('Soo.. Where do you want to travel? {bywater},{bree},{buckland} or {forsaken inn}?', cid)
 		if (getPlayerStorageValue(cid,2141) < 0) then
 				setPlayerStorageValue(cid,30030,29)
				setPlayerStorageValue(cid,2141,0)
		end
 	talkState[talkUser] = 1

elseif(msgcontains(msg, 'bywater') and talkState[talkUser] == 1) then
	 	selfSay('Traveling to Bywater will cost 700 gp, do you have they?.', cid)
		talkState[talkUser] = 2
elseif(msgcontains(msg, 'yes') and talkState[talkUser] == 2) then
		if(getPlayerStorageValue(cid,2139) == 0) then
			if(doPlayerRemoveMoney(cid, 700)) then
...
elseif(msgcontains(msg, 'bree') and talkState[talkUser] == 1) then
	 	selfSay('Traveling to Bywater will cost 500 gp, do you have they?.', cid)
...
elseif(msgcontains(msg, 'buckland') and talkState[talkUser] == 1) then
	 	selfSay('Traveling to Buckland will cost 600 gp, do you have they?.', cid)
		talkState[talkUser] = 4
elseif(msgcontains(msg, 'yes') and talkState[talkUser] == 4) then
		if(getPlayerStorageValue(cid,2138) == 0) then
			if(doPlayerRemoveMoney(cid, 600)) then
				destination3 = Position(395, 1009, 6)
				npcHandler:releaseFocus(cid)
...
elseif(msgcontains(msg, 'forsaken inn') and talkState[talkUser] == 1) then
	 	selfSay('Traveling to the Forsaken Inn will cost 400 gp, do you have they?.', cid)
		talkState[talkUser] = 5
elseif(msgcontains(msg, 'yes') and talkState[talkUser] == 5) then
		if(getPlayerStorageValue(cid,2140) == 0) then
			if(doPlayerRemoveMoney(cid, 400)) then
				destination4 = Position(874, 1035, 7)
				npcHandler:releaseFocus(cid)
