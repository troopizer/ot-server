...
condition:setConditionParam(condition, CONDITION_PARAM_SKILL_DISTANCEPERCENT, 110)
...
function onCastSpell(creature, variant, isHotkey)
	if(getPlayerStorageValue(creature,2028) == 0) then
		if(getPlayerItemCount(creature,2802) >= 1) then
			doPlayerRemoveItem(creature, 2802, 1)
